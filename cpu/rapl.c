// This Code was adapted from the "rapl_plot" program
// by Vince Weaver
...
int main (int argc, char **argv)
{
	...
	while(1) {
		...
		/* Stop Counting */
		after_time=omp_get_wtime();
		retval = PAPI_stop( EventSet, values);
		if (retval != PAPI_OK) {
			fprintf(stderr, "PAPI_start() failed\n");
		}

		total_time=((double)(after_time-start_time));
		elapsed_time=((double)(after_time-before_time));
